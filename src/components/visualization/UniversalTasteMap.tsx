...
interface UniversalTasteMapProps {
  data: DataPoint[];
  onPointClick?: (category: string, id: string) => void;
}

// Category colors
const categoryColors: Record<string, string> = {
  wine: '#722F37',      // Deep red
  whiskey: '#C68E17',   // Amber
  coffee: '#6F4E37',    // Brown
  tea: '#228B22',       // Green
  cheese: '#D2B48C',    // Tan
  beer: '#DAA520',      // Gold
...
export function UniversalTasteMap({
  data,
  onPointClick
}: UniversalTasteMapProps) {
  const navigate = useNavigate();

  const handleClick = (point: DataPoint) => {
    if (onPointClick) {
      onPointClick(point.category, point.id);
    } else {
      navigate(`/${point.category}/${point.id}`);
    }
  };
    ...
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          ...
          >
            {point.category} &middot; {point.subcategory}
          </p>
            ...
            >
              ...
              <Scatter
                ...
                shape={(props: any) => {
                  ...
                  const fill = categoryColors[point?.category] || 'var(--accent-gold)';
                  return (
                    <CustomDot
                      cx={props.cx}
                      cy={props.cy}
              ...
              >
                {data.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={categoryColors[entry.category] || 'var(--accent-gold)'}
                  />
                ))}
              </Scatter>
            </ScatterChart>
